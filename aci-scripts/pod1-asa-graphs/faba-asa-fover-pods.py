...
################################################################################
"""
Script registers ASA context L4-7 device, which resides on an ASA failover setup.
-using multiple-contexts requires that cluster device IP is Admin context IP.
-active and standby IPs are used to populate user context info as HA device
-some of those IPs are pre-configured before the for loop
"""
